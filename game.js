// ── DEMO MODE ─────────────────────────────────────────────────────────────────
//  Set to true to cap gameplay at 12 months and show the upsell screen.
const DEMO_MODE = true;
const DEMO_MAX_MONTHS = 12;
...
const gameState = {
  month:      1,
  maxMonths:  DEMO_MODE ? DEMO_MAX_MONTHS : 36,
...
const MIDGAME_CHECKPOINTS = DEMO_MODE ? new Set([]) : new Set([13, 25, 27]); // months after: 12, 24, 26
 ...
 */
// In demo mode, only these 4 mayor IDs are playable. The rest show as locked.
const DEMO_UNLOCKED_MAYORS = new Set(['businessleader', 'communitybuilder', 'faithleader', 'environmentalist' ]);
// Note: use the actual IDs from the MAYORS array
const DEMO_PLAYABLE_IDS = new Set(['businessleader', 'communitybuilder', 'faithleader', 'environmental']);

function buildMayorGrid() {
  MAYORS.forEach(mayor => {
    const card = document.createElement('div');
    const isLocked = DEMO_MODE && !DEMO_PLAYABLE_IDS.has(mayor.id);
    card.className  = 'mayor-card' + (isLocked ? ' mayor-card--locked' : '');
    ...
    if (isLocked) {
      card.innerHTML = `
        <div class="mayor-lock-overlay">
          <span class="mayor-lock-icon">🔒</span>
          <span class="mayor-lock-label">Full Version</span>
        </div>
      ...
      `;
      // Locked cards do nothing on click
      card.addEventListener('click', () => {
        showToast('Unlock this mayor in the full version — Teach With Reach');
      });
    } else {
      card.innerHTML = `
        <div class="mayor-portrait-wrap">
...
/**
 * showDemoScreen()
 * Shown after 12 months in demo mode. Presents the upsell screen.
 */
...
function restartGame() {
  // Reset state
  gameState.month           = 1;
  gameState.maxMonths       = DEMO_MODE ? DEMO_MAX_MONTHS : 36;
  gameState.term            = 1;
  ...
  // Hide demo screen if visible
  const demoScreen = document.getElementById('demoScreen');
  if (demoScreen) demoScreen.classList.remove('visible');
...
// Demo restart button
const demoRestartBtn = document.getElementById('demoRestartBtn');
if (demoRestartBtn) {
