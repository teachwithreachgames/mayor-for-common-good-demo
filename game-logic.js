...
const DEMO_END_TURN   = 24;    // DEMO: January 1930 — ends after the crash
const IS_DEMO         = true;  // Set to false for the full version
...
function nextTurn() {
  ...
  // DEMO END: intercept at turn 25 (February 1930) — after the crash has settled
  if (IS_DEMO && state.turn > DEMO_END_TURN) {
    renderAll();
    stopAllAudio();
    showDemoEndScreen();
    return;
  ...
  if (!IS_DEMO && closingMonth === 11) {
    // In demo mode, skip the annual report/reflection journal entirely
  ...
  if (!IS_DEMO && checkDecisionPrompts()) return;
...
function renderProgressBar() {
  const endTurn = IS_DEMO ? DEMO_END_TURN : GAME_END_TURN;
  const pct  = Math.min(100, (state.turn / (endTurn + 1)) * 100);
  const era  = getEra();
...
function showDemoEndScreen() {
  const nw = calcNetWorth();
  const nwStr = formatDollars(Math.abs(nw));
  const portfolioVal = calcPortfolioValue();
  const crashPct = ((100 - state.indexValue) / 100 * 100).toFixed(0);

  // Build status summary
  let statusLine, statusClass;
  if (nw < 0) {
    statusLine = `You are $${nwStr} in debt — already ruined by the crash.`;
    statusClass = "demo-status-bad";
  } else if (nw < 50) {
    statusLine = `You have $${nwStr} left — barely clinging on.`;
    statusClass = "demo-status-bad";
  } else if (portfolioVal > 0) {
    statusLine = `Net worth: $${nwStr} — but your stocks are still falling.`;
    statusClass = "demo-status-mid";
  } else {
    statusLine = `Net worth: $${nwStr} — you survived the crash with something intact.`;
    statusClass = "demo-status-good";
  }

  const overlay = document.getElementById("demo-end-overlay");
  if (!overlay) return;

  document.getElementById("demo-nw-line").textContent   = statusLine;
  document.getElementById("demo-nw-line").className     = statusClass;
  document.getElementById("demo-index-drop").textContent = `${crashPct}%`;
  const nextBtn = document.getElementById("next-turn-btn");
